...
namespace COO.Application.Common
{
    public class SAPService : ISAPService
    {
        ...
        public SAPService()
        {
            _SAPConnectionString = Utilities.Constants.SystemConstants.SAPConnectionString;
        }
        ...
        public RfcConfigParameters Connector3SapRfcConnGroup(string connStr)
        {
            ...
            {
                WriteLog.Write("Connector3SapRfcConnGroup", ex);
                return null;
            }
            catch (RfcAbapException ex)
            {
        ...
        public string TestSAPCore()
        {
            var now = DateTime.Now;
            //using (var conn = new SapConnection(_SAPConnectionString))
            //{
            //    conn.Connect();
            //    using (var func = conn.CreateFunction("ZRFC_MM_GET_TOLIST"))
            //    {


            //        var data = func.Invoke<OutputParameter>(new InputParameter
            //        { 
            //            DF049 = "049",
            //            FromDate = now,
            //            ToDate = now.AddDays(1),
            //            FromTime = now.ToString("HHmmss"),
            //            ToTime = now.AddDays(1).ToString("HHmmss")
            //        });
            ...
            try
            {
                using (var conn = new RfcConnection(_SAPConnectionString))
                {
                    conn.Open();
                    using (var func = conn.CallRfcFunction("ZRFC_MM_GET_TOLIST"))
                    {
                        func.Invoke();
                    }
                    conn.Close();
        ...
        public DataTable DownloadBoom(string plant, string parentMaterial)
        {
            ...
            {
                WriteLog.Write("DownloadBoom", ex);
                return null;
            }
            catch (RfcAbapException ex)
            {
        ...
        public PartInfo GetAvailableStock(string line, string part)
        {
            ...
            {
                return null;
                throw new COOException($"GetAvailableStock: {ex.Message}");
            }
        }
